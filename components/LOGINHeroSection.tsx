...
import { getAuth, GoogleAuthProvider, signInWithPopup, getRedirectResult, signInWithEmailAndPassword, onAuthStateChanged } from "firebase/auth";
import { auth } from '../firebaseConfig';  // Make sure your Firebase auth config is correctly imported
import Link from 'next/link';

const LOGINHeroSection = () => {
    const router = useRouter();
    const [email, setEmail] = useState<string>("");  // Define email type as string
    const [password, setPassword] = useState<string>("");  // Define password type as string
    const [error, setError] = useState<string | null>(null);  // Define error state type to allow both string and null
    ...
    useEffect(() => {
        ...
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (user) {
                router.push("/dashboard");  // Redirect to dashboard if the user is already authenticated
            }
        });
