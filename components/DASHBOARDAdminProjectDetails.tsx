...
const DASHBOARDAdminProjectDetails = ({ userId, projectId }: DASHBOARDAdminProjectDetailsProps) => {
    ...
    const [newPaymentAmount, setNewPaymentAmount] = useState(null);

    const handlePaymentAmountChange = (e) => {
        setNewPaymentAmount(e.target.value);
    };

    const handlePaymentAmountSubmit = async (e) => {
        e.preventDefault();
        if (!userId || !projectId) {
            return; // Check if userId and projectId are available
        }
        ...
        try {
            ...
            await updateDoc(projectDocRef, {
                paymentAmount: parseFloat(newPaymentAmount), // Update paymentAmount with new value
            });
                ...
                <div className="flex w-full justify-center">
                    <div className="flex flex-col gap-[20px] relative w-full mx-[50px] my-[30px]">
                        <div className="inline-flex items-center gap-[30px] relative">
                            ...
                            <div className="relative font-normal text-[#ffffff66] text-base hover:cursor-pointer">Uploads</div>
